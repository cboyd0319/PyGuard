...
"""
        issues = analyze_business_logic(code)
        float_issues = [
            i for i in issues if "float" in i.message.lower() and "currency" in i.message.lower()
        ]
...
"""
        issues = analyze_business_logic(code)
        rollback_issues = [
            i for i in issues if "rollback" in i.message.lower() and "payment" in i.message.lower()
        ]
...
"""
        issues = analyze_business_logic(code)
        assert any(
            "zip bomb" in issue.message.lower() or "archive" in issue.message.lower()
            for issue in issues
        )
...
"""
        issues = analyze_business_logic(code)
        assert any(
            "xml bomb" in issue.message.lower() or "billion laughs" in issue.message.lower()
            for issue in issues
        )
...
"""
        issues = analyze_business_logic(code)
        assert any(
            "authorization" in issue.message.lower() or "permission" in issue.message.lower()
            for issue in issues
        )
...
"""
        issues = analyze_business_logic(code)
        auth_issues = [
            i for i in issues if "delete_user" in code and "authorization" in i.message.lower()
        ]
...
"""
        issues = analyze_business_logic(code)
        assert any(
            "privilege escalation" in issue.message.lower() or "admin" in issue.message.lower()
            for issue in issues
        )
...
"""
        issues = analyze_business_logic(code)
        assert any(
            "horizontal" in issue.message.lower()
            or "IDOR" in issue.message
            or "user_id" in issue.message
            for issue in issues
        )
        assert any(issue.cwe_id == "CWE-639" for issue in issues)
...
class TestPerformance:
    ...
    def test_large_file_performance(self):
        ...
        code = "\n".join([f"def function_{i}(arg): return arg * 2" for i in range(1000)])
        issues = analyze_business_logic(code)
        assert isinstance(issues, list)
