...
class TestSecretDetectionProperties:
    ...
    @given(st.text(min_size=20, max_size=50, alphabet=st.characters(
    ...
    def test_known_secret_patterns_always_detected(self, secret):
        ...
        try:
            ...
            secret_issues = [i for i in issues if 
                           'secret' in i.category.lower() or 
                           'credential' in i.category.lower() or
                           'api' in i.message.lower() or
                           'token' in i.message.lower()]
...
class TestCodeInjectionProperties:
    ...
    def test_dangerous_functions_always_detected(self, func_name):
        ...
        try:
            ...
            injection_issues = [i for i in issues if 
                              'injection' in i.category.lower() or 
                              func_name in i.message.lower()]
...
class TestIdempotencyProperties:
    ...
    def test_analysis_is_idempotent(self, notebook_data):
        ...
        try:
            ...
            for i1, i2 in zip(sorted(issues1, key=lambda x: (x.cell_index, x.line_number)),
