...
class TestGitHooksManager(unittest.TestCase):
    ...
    def test_init(self):
        ...
        assert self.manager.repo_path is not None
        assert self.manager.git_dir is not None
        assert self.manager.hooks_dir is not None
    ...
    def test_is_git_repository(self):
        ...
        assert self.manager.is_git_repository()
    ...
    def test_is_not_git_repository(self):
        ...
        try:
            ...
            assert not manager.is_git_repository()
        finally:
    ...
    def test_list_hooks_with_pyguard_hook(self):
        ...
        assert len(pyguard_hooks) == 1
        assert pyguard_hooks[0]["name"] == "pre-commit"
        assert pyguard_hooks[0]["executable"]
    ...
    def test_validate_hook_not_executable(self):
        ...
        assert "not executable" in result["issues"][0]
    ...
    def test_validate_hook_non_pyguard(self):
        ...
        assert result["exists"]
        assert result["executable"]
        assert not result["is_pyguard"]
        assert "not a PyGuard hook" in result["issues"][0]
    ...
    def test_validate_hook_pyguard_not_in_path(self, mock_which):
        ...
        assert "not found in PATH" in result["issues"][0]
