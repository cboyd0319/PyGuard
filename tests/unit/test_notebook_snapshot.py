...
class TestGoldenFileSnapshots:
    ...
    def test_eval_fix_snapshot(self, fixtures_dir: Path, fixer: NotebookFixer, tmp_path: Path):
        ...
        assert "result = ast.literal_eval(user_input)" in cell_source, "Should have safe ast.literal_eval() call"
    ...
    def test_secrets_fix_snapshot(self, fixtures_dir: Path, fixer: NotebookFixer, tmp_path: Path):
        ...
        assert "os.getenv" in cell_source or "os.environ" in cell_source or \
               "SECURITY: Removed hardcoded secret" in cell_source, \
        ...
        for line in lines:
            if 'sk-1234567890' in line:
                ...
                assert line.strip().startswith('#'), "Secret should only appear in comments, not active code"
    
    def test_torch_load_fix_snapshot(self, fixtures_dir: Path, fixer: NotebookFixer, tmp_path: Path):
    ...
    def test_pickle_fix_snapshot(self, fixtures_dir: Path, fixer: NotebookFixer, tmp_path: Path):
        ...
        assert "pickle.load" in cell_source.lower() and "warning" in cell_source.lower() or \
               "restricted" in cell_source.lower() or \
    ...
    def test_xss_fix_snapshot(self, fixtures_dir: Path, fixer: NotebookFixer, tmp_path: Path):
        ...
        assert "html.escape" in cell_source or "bleach" in cell_source or \
               "Markdown" in cell_source, \
    ...
    def test_idempotency_eval_fix(self, fixtures_dir: Path, fixer: NotebookFixer, tmp_path: Path):
        ...
        success_1, applied_1 = fixer.fix_notebook(fixed_path_1, issues_1)
    ...
    def test_notebook_structure_preservation(self, fixtures_dir: Path, fixer: NotebookFixer, tmp_path: Path):
        ...
        assert len(fixed_notebook["cells"]) >= original_cell_count, \
            "Should not remove cells"
...
class TestSnapshotRegressionSuite:
    ...
    def test_fix_preserves_cell_order(self, tmp_path: Path):
        ...
        fixer = NotebookFixer()
        success, applied = fixer.fix_notebook(path, issues)
        assert success
