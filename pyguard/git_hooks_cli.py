...
def main():
    ...
    install_parser = subparsers.add_parser(
        "install",
        help="Install PyGuard as a git hook"
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall PyGuard git hook"
    )
    ...
    list_parser = subparsers.add_parser(
        "list",
        help="List all installed git hooks"
    )
    ...
    validate_parser = subparsers.add_parser(
        "validate",
        help="Validate PyGuard git hook installation"
    )
    ...
    if args.command == "install":
        try:
            ...
            if success:
                logger.info(
                    f"✅ Successfully installed {args.type} hook",
                    category="GitHooks"
                )
                sys.exit(0)
            else:
                logger.error(
                    f"❌ Failed to install {args.type} hook",
                    category="GitHooks"
                )
                sys.exit(1)
        ...
        if success:
            logger.info(
                f"✅ Successfully uninstalled {args.type} hook",
                category="GitHooks"
            )
            sys.exit(0)
        else:
            logger.error(
                f"❌ Failed to uninstall {args.type} hook",
                category="GitHooks"
            )
            sys.exit(1)
        ...
        if not hooks:
            ...
            logger.info(
                f"Found {len(hooks)} installed hooks:",
                category="GitHooks"
            )
            for hook in hooks:
        ...
        if result["valid"]:
            logger.info(
                f"✅ {args.type} hook is valid and ready to use",
                category="GitHooks"
            )
            sys.exit(0)
        else:
        ...
        if success:
            logger.info(
                f"✅ {args.type} hook test passed",
                category="GitHooks"
            )
            sys.exit(0)
        else:
            logger.error(
                f"❌ {args.type} hook test failed",
                category="GitHooks"
            )
            sys.exit(1)
