...
class MissingAutoFixes:
        ...
        for line in lines:
            ...
            if re.search(r'\beval\s*\(', line):
                ...
                if any(pattern in line for pattern in ['json', 'dict', 'list', '[', '{', '"', "'"]):
                    ...
                    fixed_lines.append('# FIXED: eval() → ast.literal_eval() for safe literal evaluation')
                    fixed_lines.append(fixed_line)
                    self.fixes_applied.append('Code injection: eval() → ast.literal_eval()')
                else:
                    fixed_lines.append('# SECURITY WARNING: eval() detected - consider removing or using safer alternative')
                    fixed_lines.append(line)
                    self.fixes_applied.append('Code injection warning added for eval()')
            # Check for exec() usage
            elif re.search(r'\bexec\s*\(', line):
                fixed_lines.append('# SECURITY WARNING: exec() is dangerous - consider refactoring to eliminate dynamic code execution')
                fixed_lines.append(line)
    ...
    def _fix_pickle_to_json(self, content: str) -> str:
        ...
        for line in lines:
            ...
            if re.search(r'\bpickle\.loads?\s*\(', line):
                ...
                has_complex_nearby = any(complex_type in line.lower() for complex_type in ['lambda', 'class', 'function', 'object'])
                ...
                if has_simple_data and not has_complex_nearby:
                    ...
                    fixed_lines.append('# FIXED: pickle.loads() → json.loads() for safer deserialization')
                    fixed_lines.append(fixed_line)
                    self.fixes_applied.append('Unsafe deserialization: pickle → JSON')
                else:
                    fixed_lines.append('# SECURITY WARNING: pickle deserialization is unsafe - use JSON or consider signed/authenticated serialization')
                    fixed_lines.append(line)
            ...
            # Replace pickle.dumps with json.dumps
                ...
                if has_simple_data and not has_complex_nearby:
                    ...
                    fixed_lines.append('# FIXED: pickle.dumps() → json.dumps() for safer serialization')
                    fixed_lines.append(fixed_line)
                    self.fixes_applied.append('Unsafe serialization: pickle → JSON')
                else:
                    fixed_lines.append('# SECURITY WARNING: pickle serialization - consider JSON or other format')
                    fixed_lines.append(line)
        ...
        for i, line in enumerate(lines):
            ...
            if has_lxml and 'etree.parse' in line:
                ...
                if 'XMLParser' not in line:
                    fixed_lines.append('# Add safe parser: parser = etree.XMLParser(resolve_entities=False, no_network=True)')
                self.fixes_applied.append('XXE: Added safe XML parser configuration')
            # Fix xml.etree.ElementTree
            elif ('xml.etree.ElementTree' in line or 'ET.parse' in line) and 'import' not in line.lower():
                fixed_lines.append('# SECURITY: Use defusedxml for safe XML parsing')
                fixed_lines.append('# Install: pip install defusedxml')
                fixed_lines.append('# Replace: from xml.etree.ElementTree → import defusedxml.ElementTree')
                fixed_lines.append(line)
    ...
    def _fix_format_string_vulnerabilities(self, content: str) -> str:
        ...
        for line in lines:
            ...
            if '.format(' in line and any(user_input in line for user_input in ['request', 'input(', 'args', 'form']):
                fixed_lines.append('# SECURITY WARNING: User input in format string - validate and sanitize input')
    ...
    def _fix_memory_disclosure(self, content: str) -> str:
        ...
        for line in lines:
            if 'traceback.print_exc()' in line or 'traceback.format_exc()' in line:
                ...
                fixed_lines.append(f"{' ' * indent}# Use proper logging instead of exposing tracebacks")
                fixed_lines.append(f"{' ' * indent}logger.error('An error occurred', exc_info=True)  # Logs to secure location")
                self.fixes_applied.append('Memory disclosure: Replaced traceback exposure with logging')
            elif 'locals()' in line:
                # Check if it's being exposed (printed, logged, or returned)
                if any(expose in line for expose in ['print', 'log', 'return', 'str(']):
                    fixed_lines.append('# SECURITY WARNING: locals() exposure can leak sensitive data')
                    fixed_lines.append(line)
    ...
    def _add_password_validation(self, content: str) -> str:
        ...
        for line in lines:
            ...
            if any(pattern in line.lower() for pattern in ['password', 'passwd', 'pwd']) and 'len(' in line:
                if 'len(' in line and not any(strong in line for strong in ['complexity', 'strength', 'validator']):
                    ...
                    fixed_lines.append('#   - At least 1 uppercase, 1 lowercase, 1 digit, 1 special char')
            ...
            for pattern, env_name in secret_patterns:
                if re.search(pattern, line, re.IGNORECASE):
                    if not import_added and 'import os' not in content:
                        fixed_lines.insert(0, 'import os  # SECURITY: For environment variable access')
                        import_added = True
                    ...
                    if var_match:
                        ...
                        indent = len(line) - len(line.lstrip())
                        fixed_lines.append(f"{' ' * indent}# FIXED: Hardcoded secret moved to environment variable")
                        fixed_lines.append(f"{' ' * indent}{var_name} = os.environ.get('{env_name.upper()}')  # Set in environment")
                        self.fixes_applied.append(f'Hardcoded secret: {var_name} → environment variable')
                        line_fixed = True
                        break
    ...
    def _fix_api_keys_to_config(self, content: str) -> str:
        ...
        for line in lines:
            if re.search(r'api_key\s*=\s*["\'][^"\']{20,}["\']', line, re.IGNORECASE):
                ...
                fixed_lines.append(f"{' ' * indent}# Load from: config.ini, .env file, or environment variable")
        ...
        for i, line in enumerate(lines):
            ...
            if any(pattern in line for pattern in ['get_object(', 'filter(id=', 'query.get(']):
                if 'request.' in line or 'user_id' in line.lower():
                    indent = len(line) - len(line.lstrip())
                    fixed_lines.append(f"{' ' * indent}# SECURITY: Add authorization check before accessing object")
                    fixed_lines.append(f"{' ' * indent}# if not current_user.can_access(object_id):")
                    fixed_lines.append(f"{' ' * indent}#     raise PermissionError('Access denied')")
                    fixed_lines.append(line)
    ...
    def _fix_mass_assignment(self, content: str) -> str:
        ...
        for line in lines:
            ...
            if 'update(' in line and ('request.' in line or 'data' in line):
                ...
                fixed_lines.append(f"{' ' * indent}allowed_fields = ['field1', 'field2']  # Define allowed fields")
                fixed_lines.append(f"{' ' * indent}# filtered_data = {{k: v for k, v in data.items() if k in allowed_fields}}")
    ...
    def _fix_ldap_injection(self, content: str) -> str:
        ...
        result = '\n'.join(fixed_lines)
        if needs_import:
            result = 'from ldap3.utils.conv import escape_filter_chars  # SECURITY: LDAP escaping\n\n' + result
        return result
            ...
            if has_nosql and ('find(' in line or 'find_one(' in line):
                ...
                if 'f"' in content or 'f\'' in content:
                    ...
                    if 'query' in line or 'f"' in line or 'f\'' in line:
                        ...
                        fixed_lines.append('# Use query = {"field": value} instead of string concatenation')
                        fixed_lines.append(line)
    ...
    def _fix_ssrf_vulnerabilities(self, content: str) -> str:
        ...
        for line in lines:
            if 'requests.' in line and ('request.' in line or 'url' in line.lower()):
                ...
                fixed_lines.append(f"{' ' * indent}# allowlist = ['api.trusted.com', 'data.trusted.com']")
    ...
    def _fix_open_redirect(self, content: str) -> str:
        ...
        for line in lines:
            if 'redirect(' in line and ('request.' in line or 'url' in line):
                ...
                fixed_lines.append(f"{' ' * indent}# allowed_hosts = ['yourdomain.com', 'sub.yourdomain.com']")
                fixed_lines.append(f"{' ' * indent}# if urlparse(url).hostname not in allowed_hosts:")
    ...
    def _fix_unsafe_file_operations(self, content: str) -> str:
        ...
        for line in lines:
            if has_user_input and any(op in line for op in ['open(', 'Path(', 'os.path.join(']):
                ...
                fixed_lines.append(f"{' ' * indent}# Validate: path.resolve().is_relative_to(BASE_DIR)")
    ...
    def _fix_jwt_token_leakage(self, content: str) -> str:
        ...
        for line in lines:
            if 'log' in line.lower() and 'token' in line.lower():
                indent = len(line) - len(line.lstrip())
                fixed_lines.append(f"{' ' * indent}# FIXED: JWT token leakage - sanitize before logging")
                fixed_lines.append(f"{' ' * indent}# token_safe = token[:10] + '...' if token else None")
                fixed_lines.append(line)
    ...
    def _refactor_global_variables(self, content: str) -> str:
        ...
        for line in lines:
            ...
            if not in_function and '=' in line and not line.strip().startswith('#'):
                if not any(keyword in line for keyword in ['import ', 'from ', 'def ', 'class ', '__']):
                    fixed_lines.append('# REFACTOR: Consider moving to class attribute or function parameter')
                    fixed_lines.append(line)
