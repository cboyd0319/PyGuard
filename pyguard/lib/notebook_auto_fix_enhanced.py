...
class EnhancedNotebookFixer(NotebookFixer):
    ...
    def fix_notebook_with_validation(
        ...
        for issue in sorted_issues:
            ...
            metadata = self._apply_fix_with_metadata(
                cells, issue, notebook_path, timestamp
            )
            
            if metadata:
                fix_metadata.append(metadata)
                fixes_applied.append(
                    f"[{metadata.fix_id}] {metadata.explanation}"
                )
        ...
        if validate and fix_metadata:
            ...
            if validation_issues:
                ...
                self.logger.warning(
                    f"Validation failed: {validation_issues}. Rolling back..."
                )
        ...
        if fix_metadata:
            ...
            fixes_applied.append(
                f"Rollback script: {rollback_path}"
            )
    ...
    def _fix_secret_enhanced(
        ...
        if matches:
            ...
            fixed_lines.append("")
            fixed_lines.append("# PYGUARD AUTO-FIX: Replaced hardcoded secrets with environment variables")
    ...
    def _fix_code_injection_enhanced(
        ...
        if "eval(" in source:
            ...
            if self.explanation_level == "expert":
                explanation += " (only evaluates Python literals: strings, numbers, tuples, lists, dicts)"
            ...
            fixed_lines.append("# CWE-95: Improper Neutralization of Directives in Dynamically Evaluated Code")
            fixed_lines.append("")
    ...
    def _validate_fixes(self, notebook_data: Dict) -> Optional[str]:
        """
