...
class AdvancedInjectionVisitor(ast.NodeVisitor):
    ...
    def visit_AnnAssign(self, node: ast.AnnAssign):
        ...
        if node.value and self._has_user_input(node.value) and isinstance(node.target, ast.Name):
    ...
    def _has_user_input(self, node: ast.AST) -> bool:  # noqa: PLR0911 - Complex security detection requires many checks
        ...
        if isinstance(node, ast.Name):
            ...
            name_has_indicator = any(
                indicator in node.id.lower() for indicator in user_input_indicators
            )
        ...
        for child in ast.walk(node):
            if isinstance(child, (ast.Name, ast.Attribute)) and self._has_user_input(child):
                return True
    ...
    def _check_jinja2_ssti(self, node: ast.Call, func_name: str):
        ...
        if uses_jinja or is_template_call:
            ...
            elif "Template" in func_name:  # noqa: SIM102
    ...
    def _check_mako_injection(self, node: ast.Call, func_name: str):
        ...
        if ("mako" in self.imported_modules or "Template" in func_name) and "Template" in func_name and node.args and self._has_user_input(node.args[0]):
            self._create_violation(
                node,
                "INJECT002",
                "Mako Template Injection",
    ...
    def _check_django_template_injection(self, node: ast.Call, func_name: str):
        ...
        if "django.template" in self.imported_modules and "Template" in func_name and node.args and self._has_user_input(node.args[0]):
            self._create_violation(
                node,
                "INJECT003",
                "Django Template Injection",
    ...
    def _check_tornado_template_injection(self, node: ast.Call, func_name: str):
        ...
        if "tornado.template" in self.imported_modules and "Template" in func_name and node.args and self._has_user_input(node.args[0]):
            self._create_violation(
                node,
                "INJECT004",
                "Tornado Template Injection",
    ...
    def _check_other_template_engines(self, node: ast.Call, func_name: str):
        ...
        for rule_id, (engine, title) in template_engines.items():
            if (engine in func_name.lower() or engine in str(self.imported_modules).lower()) and (("Template" in func_name or "render" in func_name or "compile" in func_name) and node.args and self._has_user_input(node.args[0])):
                self._create_violation(
                    node,
                    rule_id,
                    title,
    ...
    def _check_blind_sql_injection(self, node: ast.Call, func_name: str):
        ...
        if ("execute" in func_name or "query" in func_name) and node.args:
            for arg in node.args:
                ...
                if isinstance(arg, (ast.Constant, ast.JoinedStr)):
                    ...
                    if arg_str:
                        ...
                        if any(func in arg_str for func in time_funcs) and self._has_user_input(
                            arg
                        ):
                            self._create_violation(
                                node,
    ...
    def _check_mongodb_injection(self, node: ast.Call, func_name: str):
        ...
        if ("find" in func_name or "update" in func_name or "aggregate" in func_name) and (("pymongo" in self.imported_modules or "mongo" in func_name.lower()) and (node.args or node.keywords)):
    ...
    def _check_nosql_injection(self, node: ast.Call, func_name: str):
        ...
        for rule_id, (db, title) in nosql_patterns.items():
            if db in self.imported_modules or db in func_name.lower():  # noqa: SIM102
                if any(  # noqa: SIM102
                    method in func_name for method in ["query", "execute", "search", "get", "put"]
                ):
                    if (node.args and self._has_user_input(node.args[0])) or (
                        node.keywords
                        and any(self._has_user_input(kw.value) for kw in node.keywords)
                    ):
                        self._create_violation(
                            node,
                            rule_id,
    ...
    def _check_yaml_unsafe_load(self, node: ast.Call, func_name: str):
        ...
        if "yaml.load" in func_name or "yaml.unsafe_load" in func_name:
            ...
            if func_name == "yaml.load" and not self._get_keyword_arg(node, "Loader"):
                self._create_violation(
                    node,
                    "INJECT026",
    ...
    def _check_xml_deserialization(self, node: ast.Call, func_name: str):
        ...
        if has_xml_module or has_lxml_module:
            ...
            if any(parser in func_name for parser in unsafe_parsers):
                ...
                entities_disabled = (
                    resolve_entities
                    and isinstance(resolve_entities, ast.Constant)
                    and resolve_entities.value is False
                )
    ...
    def _check_path_traversal(self, node: ast.Call, func_name: str):
        ...
        if any(op in func_name for op in file_ops) and node.args and self._has_user_input(node.args[0]):
    ...
    def _check_ldap_injection(self, node: ast.Call, func_name: str):
        ...
        if ("ldap" in self.imported_modules or "ldap" in func_name.lower()) and "search" in func_name and node.args:
    ...
    def _check_xpath_injection(self, node: ast.Call, func_name: str):
        ...
        if ("xpath" in func_name.lower() or "lxml" in self.imported_modules) and node.args and self._has_user_input(node.args[0]):
            self._create_violation(
                node,
                "INJECT030",
    ...
    def _check_csv_injection(self, node: ast.Call, func_name: str):
        ...
        if ("csv.writer" in func_name or "writerow" in func_name) and node.args:
            for arg in node.args:
                if self._has_user_input(arg):
                    self._create_violation(
                        node,
    ...
    def _check_latex_injection(self, node: ast.Call, func_name: str):
        ...
        # Check if calling subprocess with LaTeX commands
        elif ("subprocess" in func_name or "os.system" in func_name) and node.args:
            ...
            if isinstance(first_arg, ast.List) and first_arg.elts:
                ...
                if isinstance(cmd_element, ast.Constant) and isinstance(cmd_element.value, str) and any(cmd in cmd_element.value.lower() for cmd in latex_commands):
    ...
    def _check_image_processing_injection(self, node: ast.Call, func_name: str):
        ...
        # Check if calling subprocess with ImageMagick commands
        elif "subprocess" in func_name or "os.system" in func_name:  # noqa: SIM102
            ...
            if node.args:
                ...
                if isinstance(first_arg, ast.List) and first_arg.elts:
                    cmd_element = first_arg.elts[0]
                    if isinstance(cmd_element, ast.Constant) and isinstance(cmd_element.value, str) and any(cmd == cmd_element.value for cmd in image_commands) and len(first_arg.elts) > 1:
    ...
    def _check_archive_extraction(self, node: ast.Call, func_name: str):
        ...
        if ("extractall" in func_name or "extract" in func_name) and ("zipfile" in self.imported_modules or "tarfile" in self.imported_modules):
            self._create_violation(
                node,
                "INJECT034",
                "Archive Extraction Vulnerability (Zip Slip)",
    ...
    def _check_subprocess_shell(self, node: ast.Call, func_name: str):
        ...
        if "subprocess" in func_name:
            ...
            if shell_arg and isinstance(shell_arg, ast.Constant) and shell_arg.value is True and node.args and self._has_user_input(node.args[0]):
                self._create_violation(
                    node,
                    "INJECT035",
