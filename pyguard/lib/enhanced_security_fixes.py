...
class EnhancedSecurityFixer:
    ...
    def _fix_yaml_safe_load(self, content: str) -> str:
        ...
        if "yaml.load(" in content:
            ...
            for line in lines:
                ...
                if line.strip().startswith('#') or ('yaml.load(' in line and ('"' in line or "'" in line)):
    ...
    def _fix_comparison_to_none(self, content: str) -> str:
        ...
        if len(self.fixes_applied) > 0:
            self.logger.info(
                f"Applied safe fix ({fix_id}): comparison to None",
                category="Quality"
            )
    ...
    def _fix_insecure_random_safe(self, content: str) -> str:
        ...
        if "import random" in content and "import secrets" not in content:
            has_security_context = any(
                ...
                for word in ["password", "token", "key", "secret", "auth"]
            )
            ...
            if has_security_context:
                ...
                self.logger.info(
                    "Applied safe fix (import_sorting): added secrets import",
                    category="Security"
                )
    ...
    def _fix_path_traversal_validated(self, content: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if "os.path.join" in line:
                has_user_input = any(
                    indicator in line
                    for indicator in ["request", "input", "user", "param", "arg"]
                )
                ...
                if has_user_input:
                    ...
                    if var_match:
                        ...
                        self.fixes_applied.append(f"Path traversal: added validation for {var_name}")
                        modified = True
                        continue
        ...
        if modified:
            self.logger.info(
                f"Applied unsafe fix ({fix_id}): path traversal validation",
                category="Security"
            )
