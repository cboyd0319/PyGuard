...
class BlockchainSecurityVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        ...
        if self.has_web3:
            vulnerable_patterns = [
                "approve", "transferFrom", "swap", "trade",
                "bid", "auction", "commit"
            ]
            ...
            if any(pattern in node.name.lower() for pattern in token_patterns):
                ...
                for child in ast.walk(node):
                    if isinstance(child, ast.Assign):
                        if isinstance(child.value, ast.BinOp) and isinstance(child.value.op, (ast.Add, ast.Mult, ast.Sub)):
                            violation = RuleViolation(
    ...
    def _check_reentrancy(self, node: ast.Call) -> None:
        ...
        if isinstance(node.func, ast.Attribute):
            ...
            external_call_patterns = [
                "call", "send", "transfer", "delegatecall",
                "callcode", "staticcall"
            ]
    ...
    def _check_integer_overflow(self, node: ast.Call) -> None:
        ...
        if isinstance(node.func, ast.Name):
            ...
            if any(token_word in func_name.lower() for token_word in 
                   ["transfer", "mint", "burn", "balance", "allowance", "approve"]):
    ...
    def _check_insecure_randomness(self, node: ast.Call) -> None:
        ...
        if isinstance(node.func, ast.Attribute):
            ...
            weak_sources = [
                "random", "randint", "choice",  # Python random module
                "block.timestamp", "block.number", "blockhash"  # Solidity patterns
            ]
            
            if func_name in weak_sources or (self.has_random_import and 
    ...
    def _check_private_key_exposure(self, node: ast.Assign) -> None:
        ...
        for target in node.targets:
            if isinstance(target, ast.Name):
                ...
                if any(key_word in var_name for key_word in 
                       ["private_key", "privkey", "secret_key", "wallet_key"]):
    ...
    def _check_seed_phrase_leakage(self, node: ast.Assign) -> None:
        ...
        for target in node.targets:
            if isinstance(target, ast.Name):
                ...
                if any(seed_word in var_name for seed_word in 
                       ["mnemonic", "seed", "seed_phrase", "recovery", "bip39"]):
    ...
    def _check_oracle_manipulation(self, node: ast.Call) -> None:
        ...
        if isinstance(node.func, ast.Attribute):
            ...
            oracle_patterns = [
                "getPrice", "get_price", "latestAnswer",
                "getLatestPrice", "getRoundData"
            ]
    ...
    def _check_nft_metadata_injection(self, node: ast.Call) -> None:
        ...
        if isinstance(node.func, ast.Attribute):
            ...
            nft_patterns = [
                "mint", "setTokenURI", "set_token_uri",
                "setMetadata", "set_metadata", "updateMetadata"
            ]
