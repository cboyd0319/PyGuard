...
class AnalysisCache:
    ...
    def _load_cache(self):
        ...
        try:
            if self.cache_file.exists():
                ...
                self.logger.debug(
                    f"Loaded {len(self.cache)} cache entries",
                    category="Cache"
                )
    ...
    def _save_cache(self):
        ...
        try:
            ...
            self.logger.debug(
                f"Saved {len(self.cache)} cache entries",
                category="Cache"
            )
        except Exception as e:
            self.logger.error(
    ...
    def set(self, file_path: Path, data: Any):
        ...
        self.logger.debug(
            f"Cached analysis for {file_path}",
            category="Cache"
        )
    ...
    def invalidate(self, file_path: Path):
        ...
        if file_path_str in self.cache:
            ...
            self.logger.debug(
                f"Invalidated cache for {file_path}",
                category="Cache"
            )
...
class ConfigCache:
    ...
    def get(self, config_path: Path) -> Optional[Any]:
        ...
        try:
            ...
            if current_mtime == cached_mtime:
                self.logger.debug(
                    f"Config cache hit for {config_path}",
                    category="Cache"
                )
            ...
            self.logger.warning(
                f"Error checking config cache: {str(e)}",
                category="Cache"
            )
            return None
    ...
    def set(self, config_path: Path, config: Any):
        ...
        try:
            ...
            self.logger.debug(
                f"Cached config for {config_path}",
                category="Cache"
            )
        except Exception as e:
            self.logger.warning(
                f"Error caching config: {str(e)}",
                category="Cache"
            )
    ...
    def invalidate(self, config_path: Path):
        ...
        if config_path_str in self.cache:
            ...
            self.logger.debug(
                f"Invalidated config cache for {config_path}",
                category="Cache"
            )
    ...
    def clear(self):
        ...
        self.logger.info(
            "Cleared all config cache entries",
            category="Cache"
        )
