...
class SQLAlchemySecurityVisitor(ast.NodeVisitor):
    ...
    def _check_connection_string_assignment(self, node: ast.Assign) -> None:
        ...
        for target in node.targets:
            if isinstance(target, ast.Name):
                ...
                if any(db_word in var_name for db_word in 
                       ["connection", "conn_str", "db_url", "database_url"]):
    ...
    def _check_relationship_injection(self, node: ast.ClassDef) -> None:
        ...
        is_model = any(
            (isinstance(base, ast.Name) and base.id in ["Base", "DeclarativeBase"]) or
                        ...
                        if decorator.attr == "hybrid_property":
                            ...
                            for subnode in ast.walk(item):
                                if isinstance(subnode, ast.Attribute):
                                    ...
                                    if any(sensitive in attr_name for sensitive in 
                            ...
                            if keyword.arg in ["default", "server_default"]:
                                ...
                                if isinstance(keyword.value, ast.Constant):
                                    if isinstance(keyword.value.value, str):
                                        ...
                                        if any(sensitive in value_lower for sensitive in 
