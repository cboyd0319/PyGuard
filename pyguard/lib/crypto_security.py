...
class CryptoSecurityVisitor(ast.NodeVisitor):
    ...
    def _check_deprecated_algorithms(self, node: ast.Call, func_name: str):
        ...
        deprecated_algos = {
            'DES': ['Crypto.Cipher.DES', 'DES.new', 'cryptography.hazmat.primitives.ciphers.algorithms.DES'],
            ...
            if node.args:
                ...
                if isinstance(key_size_node, ast.Constant):
                    ...
                    if isinstance(key_size, int) and key_size < 2048:
                        self._create_violation(
                            node, "CRYPTO002", "Weak RSA Key Size",
            ...
            if key_arg and isinstance(key_arg, ast.Constant):
                if isinstance(key_arg.value, (str, bytes)):
                    ...
                    if key_len < 16:  # 128 bits
                        self._create_violation(
                            node, "CRYPTO002", "Weak AES Key Size",
            ...
            if insecure_func in func_name:
                ...
                if 0 <= line_num < len(self.source_lines):
                    ...
                    if any(keyword in line for keyword in context_keywords):
                        self._create_violation(
                            node, "CRYPTO003", "Insecure Random Generation",
    ...
    def _check_weak_password_hashing(self, node: ast.Call, func_name: str):
        ...
        line_num = node.lineno - 1 if hasattr(node, 'lineno') else 0
        if 0 <= line_num < len(self.source_lines):
            context = '\n'.join(self.source_lines[max(0, line_num-2):min(len(self.source_lines), line_num+3)])
            context_lower = context.lower()
            
            is_password_context = any(indicator in context_lower for indicator in password_indicators)
            
            if is_password_context:
                for weak_hash in weak_hash_funcs:
                    if f'hashlib.{weak_hash}' in func_name or f'{weak_hash}(' in func_name:
                        self._create_violation(
                            node, "CRYPTO004", "Weak Password Hashing",
    ...
    def _check_ecb_mode(self, node: ast.Call, func_name: str):
        ...
        if 'AES.new' in func_name or 'DES.new' in func_name or 'Cipher' in func_name:
            ...
            if mode_arg:
                mode_str = self._get_attr_chain(mode_arg) if isinstance(mode_arg, ast.Attribute) else str(mode_arg)
                if 'ECB' in mode_str or 'MODE_ECB' in mode_str:
                    self._create_violation(
                        node, "CRYPTO005", "ECB Mode Usage",
    ...
    def _check_hardcoded_keys(self, node: ast.Assign):
        ...
        for target in node.targets:
            if isinstance(target, ast.Name):
                ...
                key_indicators = ['key', 'secret', 'aes_key', 'rsa_key', 'private_key', 'encryption_key']
                
                if any(indicator in var_name for indicator in key_indicators):
                    if isinstance(node.value, ast.Constant):
                        if isinstance(node.value.value, (str, bytes)) and len(str(node.value.value)) >= 8:
                            self._create_violation(
                                node, "CRYPTO008", "Hardcoded Encryption Key",
    ...
    def _check_disabled_cert_validation(self, node: ast.Call, func_name: str):
        ...
        if 'requests.get' in func_name or 'requests.post' in func_name or 'urllib.request' in func_name:
            ...
            if verify_arg and isinstance(verify_arg, ast.Constant):
                if verify_arg.value is False:
                    self._create_violation(
                        node, "CRYPTO014", "Disabled Certificate Validation",
    ...
    def _create_violation(
        ...
        description: str, recommendation: str, severity: RuleSeverity,
        cwe_id: str, compliance: str
    ):
